...
BASE64_PATTERN = re.compile(
    r"^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$", re.I
)
...
class GPGTestCase(unittest.TestCase):
    ...
    def test_environment(self):
        ...
        hd = self.homedir
        self.assertTrue(
            os.path.exists(hd) and os.path.isdir(hd),
    ...
    def generate_key(
        self, first_name, last_name, domain, passphrase=None, with_subkey=True
    ):
        "Generate a key"
        params = {
            "Key-Type": "DSA",
    ...
    def test_key_generation_with_colons(self):
        ...
        self.assertEqual(
            uid,
            "urn:uuid:731c22c4-830f-422f-80dc-14a9fdae8c19 "
    ...
    def test_add_subkey(self):
        ...
        master_key = self.generate_key(
            "Charlie", "Clark", "gamma.com", passphrase="123", with_subkey=False
        )
    ...
    def test_deletion_subkey(self):
        ...
        result = self.gpg.delete_keys(
            subkey.fingerprint, secret=True, passphrase="123", exclamation_mode=True
        )
    ...
    def test_list_keys_after_generation(self):
        ...
        self.assertTrue(
            is_list_with_len(public_keys_sigs, 1), "1-element list expected"
        )
        ...
        gpg = gnupg.GPG(
            gnupghome=hd, gpgbinary=GPGBINARY, keyring=pkn, secret_keyring=skn
        )
    ...
    def test_key_trust(self):
        ...
        self.assertRaises(
            ValueError, gpg.trust_keys, "NO_SUCH_FINGERPRINT", "TRUST_NEVER"
        )
    ...
    def test_list_signatures(self):
        ...
        self.assertTrue(
            ("BC6F350FB3D04076", "Joshua Calvert (A test user) <jc@example.com>", "13x")
            in sigs
        )
        logger.debug("testing subkey self-signature")
        self.assertTrue(
            ("BC6F350FB3D04076", "Joshua Calvert (A test user) <jc@example.com>", "18x")
            in sigs
        )
        ...
        self.assertTrue(
            (
                "057CCF658C074FDA",
                "Winston Smith (A test user) <winston@example.com>",
                "10x",
            )
            in sigs
        )
    ...
    def test_encryption_and_decryption(self):
        ...
        self.assertRaises(
            ValueError, gpg.encrypt, data, None, passphrase="bbr\x00own", symmetric=True
        )
        self.assertRaises(
            ValueError, gpg.encrypt, data, None, passphrase="bbr\rown", symmetric=True
        )
        self.assertRaises(
            ValueError, gpg.encrypt, data, None, passphrase="bbr\nown", symmetric=True
        )
    ...
    def test_import_and_export(self):
        ...
        self.assertTrue(
            ascii.find("PGP PUBLIC KEY BLOCK") >= 0, "Exported key should be public"
        )
        ...
        self.assertTrue(
            ascii.find("PGP PRIVATE KEY BLOCK") >= 0, "Exported key should be private"
        )
        binary = gpg.export_keys(
            key.fingerprint, True, armor=False, passphrase=passphrase
        )
    ...
    def test_signature_verification(self):
        ...
        self.assertEqual(
            key.fingerprint, verified.fingerprint, "Fingerprints must match"
        )
        ...
        sig = self.gpg.sign_file(
            data_file, keyid=key.fingerprint, passphrase="aable", detach=True
        )
        self.assertEqual(0, sig.returncode, "Non-zero return code")
        data_file.close()
        self.assertTrue(sig, "File signing should succeed")
    ...
    def test_subkey_signature_file(self):
        ...
        if subkey.fingerprint != verified.fingerprint:
            ...
            logger.debug("ver: %r", verified.fingerprint)
        self.assertEqual(
            subkey.fingerprint, verified.fingerprint, "Fingerprints must match"
        )
    ...
    def do_file_encryption_and_decryption(self, encfname, decfname):
        ...
        try:
            ...
            edata = self.gpg.encrypt_file(
                file, [andrew, barbara], armor=False, output=encfname
            )
            ...
            if gnupg._py3k:
                ...
                ddata = self.gpg.decrypt_file(
                    efile, passphrase="bbrown", output=decfname
                )
        ...
        if os.path.exists("messages.json"):
            ...
            try:
                ...
                for badout, message in cases:
                    stream = gnupg._make_binary_stream(data, self.gpg.encoding)
                    edata = self.gpg.encrypt_file(
                        stream, barbara, armor=False, output=badout
                    )
    ...
    def disabled_test_signing_with_uid(self):  # pragma: no cover
        ...
        try:
            ...
            signed = self.gpg.sign_file(
                signfile, keyid=uid, passphrase="aable", detach=True
            )
        finally:
            signfile.close()
    ...
    def test_invalid_fileobject(self):
        ...
        with self.assertRaises(TypeError) as ec:
            self.gpg.decrypt_file(
                "foobar.txt", passphrase="", output="/tmp/decrypted.txt"
            )
